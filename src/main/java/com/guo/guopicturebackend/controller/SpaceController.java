...
import com.guo.guopicturebackend.model.dto.space.*;
...
public class SpaceController {
    ...
    public BaseResponse<Page<SpaceVO>> listSpaceVOByPage(@RequestBody SpaceQueryRequest spaceQueryRequest,
        ...
        // 如果查询指定用户的空间，需要验证权限：只能查询自己的空间，或者是管理员
        Long queryUserId = spaceQueryRequest.getUserId();
        if (queryUserId != null && !queryUserId.equals(loginUser.getId()) && !userService.isAdmin(loginUser)) {
            throw new BusinessException(ErrorCode.NO_AUTH_ERROR, "无权限查询其他用户的空间");
        }
        // 查询数据库
        Page<Space> spacePage = spaceService.page(new Page<>(current, size),
                spaceService.getQueryWrapper(spaceQueryRequest));
